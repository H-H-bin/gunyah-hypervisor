...
smccc_handle_hypercall_wrapper(smccc_function_id_t smc_id, bool is_hvc)
{
	...
	register_t(*regs)[SMCCC_1_2_RETS] =
		(register_t(*)[SMCCC_1_2_RETS])(
			void *)&current->vcpu_regs_gpr.x[0];
	...
	case SMCCC_VENDOR_HYP_FUNCTION_CLASS_PLATFORM_CALL:
		handled = smccc_handle_smc_platform_call(regs, is_hvc);
		break;
		...
		if (is_fast && is_smc64) {
			uint32_t hyp_num =
				smccc_vendor_hyp_function_id_get_function(
					&smc_type);
			smccc_hypercall_table_wrapper(
				hyp_num, &current->vcpu_regs_gpr.x[0]);
		} else {
	...
	case SMCCC_VENDOR_HYP_FUNCTION_CLASS_SERVICE:
		if (is_fast && !is_smc64) {
			...
			case SMCCC_VENDOR_HYP_FUNCTION_CALL_UID:
				(*regs)[0] = SMCCC_GUNYAH_UID0;
				(*regs)[1] = SMCCC_GUNYAH_UID1;
				(*regs)[2] = SMCCC_GUNYAH_UID2;
				(*regs)[3] = SMCCC_GUNYAH_UID3;
				break;
			case SMCCC_VENDOR_HYP_FUNCTION_REVISION:
				(*regs)[0] = (register_t)hyp_api_info_raw(
