			    ...
			    platform_msi_device_id_t device_id)
{
	...
	list_foreach_container (device, &vgic_its->device_list,
		...
		if ((atomic_load_consume(&device->vgic_its_owner) ==
		     vgic_its) &&
		      ...
		      platform_msi_event_id_t event, bool inv, bool discard)
{
	...
	platform_msi_device_t *phys_device =
		vgic_its_lookup_phys_device(vgic_its, device);
	if (phys_device == NULL) {
		...
		platform_msi_id_set_its_index(&ret.r, vgic_its->phys_index);
...
vgic_its_cmd_sync(vgic_its_t *vgic_its, vgic_its_rdbase_t rdbase)
{
	...
	count_result_t count_r =
		gicv3_its_vsync(vgic_its->phys_index, vcpu_r.r);
	if (count_r.e != OK) {
		err = count_r.e;
		goto out_rcu;
	}
	...
	// FIXME:
	err = gicv3_its_wait(vgic_its->phys_index, count_r.r);
