...
extern const char soc_fvp_entry_cold_secondary;
extern const char soc_fvp_entry_warm;

CPULOCAL_DECLARE_STATIC(bool, cpu_started);

void
soc_fvp_handle_boot_cpu_cold_init(cpu_index_t cpu)
{
...
platform_cpu_on(cpu_index_t cpu)
{
	...
	uintptr_t   entry_virt =
		  CPULOCAL_BY_INDEX(cpu_started, cpu)
			  ? (uintptr_t)&soc_fvp_entry_warm
			  : (uintptr_t)&soc_fvp_entry_cold_secondary;
...
psci_smc_cpu_suspend_arg(register_t power_state) REQUIRE_PREEMPT_DISABLED
{
	...
	paddr_t entry_phys =
		partition_image_virt_to_phys((uintptr_t)&soc_fvp_entry_warm);
