...
{
	// FIXME: The RAM memory size is currently hardcoded. We need to find a
			   ...
			   qcbor_enc_ctxt_t *qcbor_enc_ctxt)
{
	...
	// FIXME: The memory layout is hardcoded here. We need to find a
	// better solution for this, possibly by using a system-device-tree
	...
	// Not the same as Qemu, set this like for QCom or fail later
	QCBOREncode_AddUInt64ToMap(qcbor_enc_ctxt, "device_me_base", 0U);
	...
	// Derive a memextent for the GIC to effectively remove it from the
	// device memextent we provide to the rootvm.
	memextent_ptr_result_t gic_me_ret =
		memextent_derive(me, PLATFORM_GIC_BASE, PLATFORM_GIC_SIZE,
				 MEMEXTENT_MEMTYPE_DEVICE, PGTABLE_ACCESS_RW,
				 MEMEXTENT_TYPE_BASIC);
	if (gic_me_ret.e != OK) {
		panic("Failed creation of GIC memextent");
	}

	QCBOREncode_OpenArrayInMap(qcbor_enc_ctxt, "gic_xlate_me");
	// Derive memextents for the GICA or the GITS translate pages to allow
	// them to be mapped to non-HLOS VMs
#if defined(PLATFORM_GICA_BASE) && !VGIC_HAS_LPI
	memextent_ptr_result_t gica_me_ret = memextent_derive(
		gic_me_ret.r, PLATFORM_GICA_BASE - PLATFORM_GIC_BASE,
		PGTABLE_VM_PAGE_SIZE, MEMEXTENT_MEMTYPE_DEVICE,
		PGTABLE_ACCESS_RW, MEMEXTENT_TYPE_BASIC);
	if (gica_me_ret.e != OK) {
		panic("Failed creation of GICA memextent");
	}
	obj_ptr.memextent = gica_me_ret.r;
	capid_ret	  = cspace_create_master_cap(root_cspace, obj_ptr,
						     OBJECT_TYPE_MEMEXTENT);
	if (capid_ret.e != OK) {
		panic("Error create memextent cap id.");
	}

	QCBOREncode_AddUInt64(qcbor_enc_ctxt, capid_ret.r);

#elif VGIC_HAS_LPI
	for (count_t i = 0U; i < PLATFORM_GITS_COUNT; i++) {
		memextent_ptr_result_t gits_me_ret = memextent_derive(
			gic_me_ret.r,
			PLATFORM_GITS_BASE - PLATFORM_GIC_BASE +
				((size_t)i << GITS_STRIDE_SHIFT) +
				offsetof(gits_t, xlate),
			PGTABLE_VM_PAGE_SIZE, MEMEXTENT_MEMTYPE_DEVICE,
			PGTABLE_ACCESS_RW, MEMEXTENT_TYPE_BASIC);
		if (gits_me_ret.e != OK) {
			panic("Failed creation of GICA memextent");
		}
		obj_ptr.memextent = gits_me_ret.r;
		capid_ret = cspace_create_master_cap(root_cspace, obj_ptr,
						     OBJECT_TYPE_MEMEXTENT);
		if (capid_ret.e != OK) {
			panic("Error create memextent cap id.");
...
#if !SMMU_V3_PASSTHROUGH
	// Remove SMMU from me
	// Note: With pass-through we can give linux direct access to the device
	// for testing
	memextent_ptr_result_t smmu_me_ret = memextent_derive(
		me, PLATFORM_SMMU_V3_BASE, PLATFORM_SMMU_V3_SIZE,
		MEMEXTENT_MEMTYPE_DEVICE, PGTABLE_ACCESS_RW,
		MEMEXTENT_TYPE_BASIC);
	if (smmu_me_ret.e != OK) {
		panic("Failed creation of smmuv3 memextent");
	}
#endif

	// Derive extent for UART and share it with RM
	memextent_ptr_result_t uart_me_ret = memextent_derive(
		me, PLATFORM_HYP_UART_BASE, PLATFORM_HYP_UART_SIZE,
		MEMEXTENT_MEMTYPE_DEVICE, PGTABLE_ACCESS_RW,
		MEMEXTENT_TYPE_BASIC);
	if (uart_me_ret.e != OK) {
	...
	obj_ptr.memextent = uart_me_ret.r;
	capid_ret	  = cspace_create_master_cap(root_cspace, obj_ptr,
						     OBJECT_TYPE_MEMEXTENT);
	if (capid_ret.e != OK) {
		panic("Error create memextent cap id.");
