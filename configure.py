...
class AbstractBuildGraph(object):
    ...
    def __call__(self, **kwargs):
        ...
        if not self._variants:
            ...
            # Add Python dependencies for build graph generation
            self.add_gen_source(os.path.join('tools', 'build', '__main__.py'))
    ...
    def add_gen_source(self, source):
        ...
        changes. The base class calls this for all Python modules that have
        been loaded by the end of the build graph generation. The graph
        generator should call it for any other files it reads, such as
...
class NinjaBuild(AbstractBuildGraph):
    def __init__(self, root_dir, build_dir=None, **kwargs):
        ...
        if build_dir is None:
            ninja_file = os.path.join(root_dir, 'build.ninja')
            build_dir = 'build' if root_dir == '.' else root_dir
        else:
        ...
        try:
            ...
            pass
        self._mkdir_cache = {'.', os.path.relpath(build_dir, start=root_dir)}
        self._mkdir_targets = []

        super(NinjaBuild, self).__init__(build_dir=build_dir, **kwargs)

    def _make_variant(self, build_dir):
        variant = type(self)(self.root_dir, build_dir=build_dir, _parent=self)

        self._lines.append('')
        self._lines.append('subninja ' + os.path.relpath(variant._ninja_file,
                                                         start=self.root_dir))
    ...
    def __call__(self, gen_cmd=None, **kwargs):
        ...
        if not self._is_variant:
            ...
            byproducts = (os.path.join(self.root_dir, b)
                          for b in self._all_byproducts)
        ...
        for d in reversed(self._mkdir_targets):
            self.add_target([os.path.join(self.root_dir, d)], '_mkdir',
                            _is_auto_dir=True)
    ...
    def root_dir(self):
        ...
        For Ninja, it is the path to the root directory of the build.
        """
    ...
    def add_target(self, targets, rule, sources=None, depends=None,
        ...
        if always:
            build_line += ' ' + os.path.relpath(self._phony_always,
                                                start=self.root_dir) + ' '
        if requires:
            build_line += ' || '
    ...
    def add_alias(self, alias, targets):
        alias = self._expand_target_list(alias)[0]
        targets = self._expand_target_list(targets)
        self._lines.append('')
    ...
    def add_default_target(self, target, alias=False):
        self._lines.append('')
        self._lines.append('default ' + self._escape(
            os.path.relpath(target, start=self.root_dir)))

    def add_gen_source(self, source):
        self._gen_sources.add(os.path.normpath(source))

    def add_gen_output(self, output):
        self._gen_outputs.add(os.path.relpath(output, start=self.root_dir))
    ...
    def _expand_target_list(self, target_list):
        return tuple(self._escape(os.path.relpath(s, start=self.root_dir))
...
if __name__ == '__main__':
    ...
    root_dir = os.path.relpath('.', start=repo_root)
    os.chdir(repo_root)
